...
public class MP3Sound {
	...
	/**
	 * Initalizes a new MP3 playing sound system. You only need to make one of these to play all of your sounds.
	 * You do not need to worry about putting it in a new thread, because this MP3 Engine does it on its own.
	 * To use the sound system, add the clips into the system using the addClip() and addClipRelitive() methods.
	 * After the clips are added, you can play those clips by asking the sound system to .playClip(), giving it the int that addClip returns.
	 * Once a clip is playing, you can stop it using .stopClip(), giving it the instance of the sound, as well as the clip number.
	 */
	public MP3Sound(){
		clips = new ArrayList<MP3Clip>();
	 ...
	 * @return the int of the clip added. It is used in playClip()
	 */
	public int addClipRelitive(String url){
		try{
